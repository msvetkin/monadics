...
class smart_ptr {
 public:
  ...
  constexpr smart_ptr(const smart_ptr<T> &other)
    : value_(std::exchange(other.value_, nullptr)){
  }

  constexpr smart_ptr(smart_ptr<T> &&other)
    : value_(std::exchange(other.value_, nullptr)){
  }
...
struct noisy {
  ...
  noisy(std::source_location loc = std::source_location::current())
      : loc_(loc) {
    std::cout << "ctor at: " << loc.file_name() << ":" << loc.line() << " " << loc.function_name() << "\n";
    ctor++;
  }

  noisy(const noisy&, std::source_location loc = std::source_location::current())
      : loc_(loc) {
  ...
  noisy(noisy&&, std::source_location loc = std::source_location::current())
      : loc_(loc) {
